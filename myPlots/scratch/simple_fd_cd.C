// simple_fd_cd.C
//
// FD/CD 1D shape overlays from a TTree.
//
// Usage examples:
//   root -l -b -q 'scratch/simple_fd_cd.C'
//   root -l -b -q 'scratch/simple_fd_cd.C("~/data/RGM_DATA/c12_src_skim.root")'
//   root -l -b -q 'scratch/simple_fd_cd.C("input.root","srcTree","my_fd_cd_overlays.pdf","xB>1.2")'
//
// Edit variablesToPlot() below to add/remove variables.  For each variable, the
// PDF gets one e'p page and/or one e'pp page.  Each page overlays forward
// lead protons (leadTheta < 37 deg) and central lead protons (leadTheta > 45 deg),
// with both histograms scaled to unit area.  The tree is read once.
...
vector<PlotVar> variablesToPlot() {
  return {
      ...
      {"leadTheta", "leadTheta*180./TMath::Pi()", "#theta_{lead} [deg]", 0.0, 180.0, true, true},
...
void styleHist(TH1D* h, const Detector& det) {
  h->SetDirectory(nullptr);
  h->SetStats(0);
  h->SetLineColor(det.color);
  h->SetMarkerColor(det.color);
  h->SetMarkerStyle(det.marker);
  h->SetMarkerSize(0.9);
  h->SetLineWidth(3);
}
...
void drawOverlayPage(TCanvas* c,
  ...
  for (int d = 0; d < (int)detectors.size(); d++) {
    TH1D* h = hists[channelIndex][varIndex][d];
  ...
  TLegend* leg = new TLegend(0.62, 0.72, 0.93, 0.88);
  leg->SetBorderSize(0);
  leg->SetFillStyle(0);
  ...
  for (int d = 0; d < (int)detectors.size(); d++) {
    ...
    h->Draw((d == 0) ? "PE" : "PE SAME");
    leg->AddEntry(h, detectors[d].label.c_str(), "lep");
  }
  ...
  title.DrawLatex(0.12, 0.94,
                  Form("%s: %s by lead-proton detector", channel.label.c_str(), v.xTitle.c_str()));
  ...
  // if (!channel.weightExpr.empty()) note.DrawLatex(0.12, 0.77, Form("%s", channel.key.c_str()));

  leg->Draw();
  c->Print(outPath.c_str(), "pdf");
}

void drawRatioPage(TCanvas* c,
                   const string& outPath,
  ...
  vector<TH1D*> ratios;
  ratios.reserve(detectors.size());
  double ymax = 0.0;

  for (int d = 0; d < (int)detectors.size(); d++) {
    if (!hEppPmiss[d] || !hEpPmiss[d] ||
        hEppPmiss[d]->Integral() <= 0.0 || hEpPmiss[d]->Integral() <= 0.0) {
      ratios.push_back(nullptr);
      continue;
    }

    TH1D* hRatio = (TH1D*)hEppPmiss[d]->Clone(Form("h_ratio_epp_over_ep_pMiss_%s", detectors[d].key.c_str()));
    hRatio->SetDirectory(nullptr);
    hRatio->Divide(hEppPmiss[d], hEpPmiss[d], 1.0, 1.0);
    styleHist(hRatio, detectors[d]);
  ...
  if (ymax <= 0.0) {
    drawMissingPage(c, outPath, "No valid FD/CD entries found for e'pp/e'p vs p_{miss}.");
    return;
  }
  ...
  for (int d = 0; d < (int)ratios.size(); d++) {
    TH1D* hRatio = ratios[d];
    if (!hRatio) continue;
    hRatio->GetYaxis()->SetRangeUser(0.0, 1.25 * ymax);
    hRatio->Draw(drewFirst ? "PE SAME" : "PE");
    drewFirst = true;
    if (!axisHist) axisHist = hRatio;
    leg->AddEntry(hRatio, detectors[d].label.c_str(), "lep");
  }
  ...
  title.SetTextFont(62);
  title.SetTextSize(0.038);
  title.DrawLatex(0.12, 0.94, "e'pp / e'p vs p_{miss} by lead-proton detector");

  TLatex note;
  note.SetNDC();
  note.SetTextSize(0.025);
  note.DrawLatex(0.12, 0.89, "Ratio in fixed p_{miss} bins");
  if (!baseCut.empty()) note.DrawLatex(0.12, 0.85, Form("Common cut: %s", baseCut.c_str()));
...
void simple_fd_cd(const char* inputFile = "~/data/RGM_DATA/c12_src_skim.root",
  ...
  TCanvas* c = new TCanvas("c_fd_cd_overlays", "FD/CD overlays", 900, 700);
  ...
  const vector<Detector> detectors = {
      {"fd", "Forward: #theta_{lead} < 37#circ", "leadTheta*180./TMath::Pi() < 37.", kBlue + 1, 20},
      {"cd", "Central: #theta_{lead} > 45#circ", "leadTheta*180./TMath::Pi() > 45.", kRed + 1, 21},
  };
  ...
  if (pMissVarIndex >= 0) {
    hRatioEpPmiss.reserve(detectors.size());
    hRatioEppPmiss.reserve(detectors.size());
    for (int d = 0; d < (int)detectors.size(); d++) {
      TH1D* hEp = new TH1D(Form("h_ratio_den_ep_pMiss_%s", detectors[d].key.c_str()), "",
                           (int)bE_pmiss.size() - 1, bE_pmiss.data());
      TH1D* hEpp = new TH1D(Form("h_ratio_num_epp_pMiss_%s", detectors[d].key.c_str()), "",
                            (int)bE_pmiss.size() - 1, bE_pmiss.data());
  ...
  for (int cidx = 0; cidx < (int)channels.size(); cidx++) {
    for (int vidx = 0; vidx < (int)vars.size(); vidx++) {
      hists[cidx][vidx].reserve(detectors.size());
      for (int d = 0; d < (int)detectors.size(); d++) {
        const PlotVar& v = vars[vidx];
        const string hname = Form("h_%s_%s_%s",
                                  channels[cidx].key.c_str(), detectors[d].key.c_str(), v.name.c_str());
        TH1D* h = new TH1D(hname.c_str(), "", binsForChannel(v, channels[cidx]), v.xmin, v.xmax);
        h->Sumw2();
        styleHist(h, detectors[d]);
  ...
  vector<Formula*> detFormulas;
  detFormulas.reserve(detectors.size());
  for (int d = 0; d < (int)detectors.size(); d++) {
    detFormulas.push_back(new Formula(tree, Form("det_formula_%d", d), detectors[d].cutExpr));
  }
  ...
  bool formulasOK = commonCutFormula.ok() && epCutFormula.ok() && eppCutFormula.ok() &&
                    epWeightFormula.ok() && eppWeightFormula.ok();
  for (const auto* df : detFormulas) formulasOK = formulasOK && df->ok();
  ...
  if (!formulasOK) {
    ...
    for (auto* df : detFormulas) delete df;
  ...
  for (Long64_t entry = 0; entry < nEntries; entry++) {
    ...
    for (int d = 0; d < (int)detectors.size(); d++) {
      if (!passes(*detFormulas[d])) continue;

      if (pMissVarIndex >= 0) {
        const double pMissValue = varFormulas[pMissVarIndex]->eval(0.0);
        if (passEp && wEp != 0.0) hRatioEpPmiss[d]->Fill(pMissValue, wEp);
        if (passEpp && wEpp != 0.0) hRatioEppPmiss[d]->Fill(pMissValue, wEpp);
      }

      for (int vidx = 0; vidx < (int)vars.size(); vidx++) {
        const double value = varFormulas[vidx]->eval(0.0);
        if (vars[vidx].drawEp && passEp && wEp != 0.0) hists[0][vidx][d]->Fill(value, wEp);
        if (vars[vidx].drawEpp && passEpp && wEpp != 0.0) hists[1][vidx][d]->Fill(value, wEpp);
      }
    }
  }
  ...
  for (int vidx = 0; vidx < (int)vars.size(); vidx++) {
    if (vars[vidx].drawEp) {
      drawOverlayPage(c, outPath, vars[vidx], channels[0], detectors, hists, 0, vidx, baseCut);
    }
    if (vars[vidx].drawEpp) {
      drawOverlayPage(c, outPath, vars[vidx], channels[1], detectors, hists, 1, vidx, baseCut);
    }
  }
  if (pMissVarIndex >= 0) {
    drawRatioPage(c, outPath, hRatioEppPmiss, hRatioEpPmiss, detectors, baseCut, channels[0], channels[1]);
  }
