...
int main() {
  ...
  Event fdLead = baseEvent(2, 0.01, 0.02, 0.03, cfg.fdLeadRegionValue);
  fdLead.thetaLead = 25.0;
  Event cdLead = baseEvent(3, 0.01, 0.02, 0.03, cfg.cdLeadRegionValue);
  cdLead.thetaLead = 55.0;
  assert(passesCuts(fdLead, cfg));
  assert(!passesCuts(cdLead, cfg));
  cfg.leadMode = LeadMode::CD;
  assert(!passesCuts(fdLead, cfg));
  assert(passesCuts(cdLead, cfg));
  cfg.leadMode = LeadMode::BOTH;
  assert(passesCuts(fdLead, cfg));
  assert(passesCuts(cdLead, cfg));
  cfg.leadMode = LeadMode::FD;
